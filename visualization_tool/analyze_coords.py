...
if __name__ == "__main__":
    ...
    if args.v : 
        ...
        display_3d_torus_with_selected_nodes(physical_node_shape[0:3], phys_unique_3D)
        
    if args.m:
        print("\n####### Computing Hop-Bytes in the 6D Torus")
        mat = load_comm_mat(args.m)
        hop_matrix = hop_matrix_from_coords(physical_node_shape, phys_nodes_coord_6D) 
        ...
        hop_matrix_unique = hop_matrix_from_coords(physical_node_shape, phys_unique_6D)
        ...
        link_usage = compute_link_usage_with_lookup(physical_node_shape[0:3], phys_nodes_3D, mat, out_filename+"_RR")
        link_usage_tm_sub_fugaku = compute_link_usage_with_lookup(physical_node_shape[0:3], phys_nodes_3D, mat_tm_sub_fugaku, out_filename+"_tm_sub")
        link_usage_tm_phys = compute_link_usage_with_lookup(physical_node_shape[0:3], phys_nodes_3D, mat_tm_phys, out_filename+"_tm_phys")
        link_usage_scotch = compute_link_usage_with_lookup(physical_node_shape[0:3], phys_nodes_3D, mat_tm_sub_fugaku, out_filename+"_scotch")
        ...
        if args.v:
            display_3d_torus_with_selected_nodes(physical_node_shape[0:3], phys_unique_3D, link_usage, name= coord_filename + " RR", quantile = 0)
            display_3d_torus_with_selected_nodes(physical_node_shape[0:3], phys_unique_3D, link_usage_tm_sub_fugaku, name=coord_filename + " TM Sub Fugaku",quantile = 0.33)
            display_3d_torus_with_selected_nodes(physical_node_shape[0:3], phys_unique_3D, link_usage_tm_phys, name= coord_filename + " TM Phys",quantile = 0.33)
            display_3d_torus_with_selected_nodes(physical_node_shape[0:3], phys_unique_3D, link_usage_scotch, name= coord_filename + " Scotch",quantile = 0.33)
